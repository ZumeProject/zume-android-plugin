...
class Zume_Android_REST_Endpoints
{
	...
	public function add_api_routes()
	{
		...
		register_rest_route(
			$namespace, '/group/(?P<user_id>\d+)', [
				[
					'methods'  => WP_REST_Server::EDITABLE,
					'callback' => [ $this, 'post_group' ],
				],
			]
		);
	...
	public function get_user( WP_REST_Request $request )
	{
		$params = $request->get_params();
		if ( isset( $params['user_id'] ) ) {
			$result = Zume_Android_App::get_user( $params['user_id'] );
			if ( $result['status'] ) {
				return $result['data'];
			} else {
				return new WP_Error( "mark_viewed_processing_error", $result["message"], [ 'status' => 400 ] );
	...
	public function post_group( WP_REST_Request $request )
	{
		$params = $request->get_params();
		if( isset( $params['group'] ) && isset( $params['args'] ) ) {
			$result = Zume_Android_App::post_group( $params['group'], $params['args'] );
			if ( $result['status'] ) {
				return $result['data'];
			} else {
	...
	/**
	 * Get all lessons as JSON array.
	 *
	 * @param  WP_REST_Request $request is currently unused.
	 *
	 * @access public
	 * @since  0.1.0
	 ...
	 */
	public function get_lessons_all( WP_REST_Request $request )
	{
		//$params = $request->get_params();
		$result = Zume_Android_App::get_lessons_all();
		if ( $result['status'] ) {
			return $result['data'];
		} else {
			return new WP_Error( 'mark_viewed_processing_error', $result['message'], [ 'status' => 400 ] );
...
class Zume_Android_App {
	...
	public static function user_profile ( $user_id ) {
		...
		foreach($user_meta as $key => $value){
			$user_meta[$key][0] = maybe_unserialize($value[0]);
		}
...
//	error_log(print_r($user_meta, true));
	...
	public static function get_user ( $user_id ) {
		...
		$obj = (object)array('user_id' => get_current_user_id(), 'user_email' => $email);
	...
	public static function post_group ( $group, $args ){

		$data = get_user_meta(get_current_user_id(), $group);
		...
		$user = update_user_meta( get_current_user_id(), $group , $data[0] );
	...
	/**
	 * Generates an array of lesson content parsed from the HTML code generated
	 * by the application.
    *
	 * The method 'get_course_content()' generates output so we capture the
	 * output buffer (using ob_start()) and then pass the HTML to the parser.
	 * 
	 * @since 0.1.0
	 *
	 * @see class Zume_Lesson_Parser (below)
	 * @see class Zume_Course_Content (in ...)
	 *
	 * @return array with status and data generated by parsing all the lessons.
	 */
	public static function get_lessons_all () {
		$parser = new Zume_Lesson_Parser();
		$out['course'] = [];

		for ( $i = 1; $i <= 10; $i++ ) {
			ob_start();
			Zume_Course_Content::get_course_content( $i );
			$out['course'][] = $parser->parse( ob_get_contents() );
			ob_end_clean();
		}
...
/**
 * Provides functions to parse a Zume lesson document and convert it to JSON
 * for passing to the Android app.
 *
 * @since 0.1.0
 */
class Zume_Lesson_Parser {

	/**
	 * Constructor turns off annoying messages and establishes xpath variable.
	 * @since 0.1.0
	 *
	 */
	public function __construct() {
	...
	/**
	 * Returns true if the given $node has a class of $class.
	 *
	 * We don't want partial matches.
    *
	 * @since 0.1.0
	 *
	 * @param DOMElement $node The node whose class attribute we are checking.
	 * @param string $class The name of the class we are checking for.
	 * 
	 * @return boolean TRUE if $node has the specified $class; FALSE otherwise.
	 */
	private function hasClassOf( $node, $class )
	{
		if ( ! $node->hasAttribute('class') ) {
			return FALSE;
		}

		$normalizedClass = ' ' . trim( $node->getAttribute('class')) . ' ';
		return strpos( $normalizedClass, " $class " ) !== FALSE;
	}
	...
	/**
	 * Parse each child of $node.
	 *
	 * Because of our special handling of <br>'s it is convenient to have this function.
	 * We combine adjacent <br>'s as a single entry in the JSON output.
    *
	 * @since 0.1.0
	 *
	 * @return string JSON string resulting from the parsing of the children.
	 */
	private function parseChildren( $node )
	{
		...
		foreach ( $node->childNodes as $child ) {
			if ( $child->nodeName=='br' ) {
				...
				$outstr .= $this->parseElement( $child );
			}
		}
		return $outstr;
	}
	...
	/**
	 * Parse a single element. Returns TRUE if output was generated, FALSE otherwise.
	 *
	 *   NOTE: element = one of these
	 *    - {"text":string}
	 *    - {"activity-title":string} # large text in left column
	 *    - {"activity-description": [element, element, ...] # yes this is a recursive definition
	 *    - {"video":string/url}
	 *    - {"br":int} # number of blank lines in website code
	 *    - {"p": [element, element, ...]}
	 *    - {"ul": [li, li, ...]} # each list item is: [element, element, ...]
	 *    - {"ol": [li, li, ...]} # each list item is: [element, element, ...]
	 *    - {"link": {"url":string,"label":string,"style":"inline/button"}} # hyperlink
	 *    - {"center": [element, element, ...]} # contained elements are centered
    *
	 * @since 0.1.0
	 *
	 * @param DOMElement $elem JSON string resulting from the parsing of the children.
	 * @return string JSON string resulting from the parsing of the children.
	 */
	private function parseElement( $elem )
	{
		if ( null == $elem ) return;

		// handle plain text
		if ( '#text' == $elem->nodeName ) {
			$val = trim( $elem->nodeValue );
			if ( 0 == strlen( $val ) ) {
				return '';
			}
			return '{"text":'.json_encode( $val ).'},';

		// handle divs
		} elseif ( 'div' == $elem->nodeName ) {
			if ( $this->hasClassOf( $elem, 'activity-title' ) ) {
				return '{"activity-title":"' . trim( $elem->nodeValue ) . '"},';
			}
			elseif ( $this->hasClassOf( $elem, 'activity-description' ) ) {
				return '{"activity-description":[' . rtrim( $this->parseChildren( $elem ), ',' ).']},';
			}
			else {
				//echo "A blank div\n";
				//echo "Class: ".$leadWithComma.$elem->getAttribute('class')."\n";
				$outstr = $this->parseChildren( $elem );
				if ( strlen( $outstr ) > 0 ) {
					if ( $this->hasClassOf( $elem, 'center' ) ) {
						return '{"center":[' . rtrim( $outstr, ',' ) . ']},';
					}
					else {
						return $outstr;
					}
		...
		// handle videos
		} elseif ( 'iframe' == $elem->nodeName ) {
			return '{"video": "' . $elem->getAttribute( 'src' ) . '"},';

		// handle paragraphs, lists, and list items
		} elseif ( 'p' == $elem->nodeName || 'li' == $elem->nodeName || 'ol' == $elem->nodeName || 'ul' == $elem->nodeName ) {
			$node = 'p' == $elem->nodeName && $this->hasClassOf( $elem, 'center' ) ? 'center' : $elem->nodeName;
			$outstr = $this->parseChildren( $elem );
			if ( strlen( $outstr ) > 0 ) {
				return '{"' . $node . '":[' . rtrim( $outstr, ',' ) . ']},';
			}
			return '';

		// handle links
		} elseif ( 'a' == $elem->nodeName ) {
			$style = $this->hasClassOf( $elem, 'button' ) ? 'button' : 'inline';
			return '{"link": {"url":"' . $elem->getAttribute( 'href' ) . '","label":"' . trim( $elem->nodeValue ).'","style":"' . $style . '"}},';
		}
	 ...
	 * Parse a single step.
	 *
	 * NOTE: step= {"title":string,"content":[element, element, ...]}
	 *
	 * @since 0.1.0
	 *
	 * @param DOMElement $step HTML/DOM code for a single step in the lesson.
	 * @return string JSON string resulting from the parsing of the step.
	 */
	private function parseStep( $step )
	{
		$result = $this->xpath->query( './/div[contains(concat(" ", normalize-space(@class), " "), " step-title ")]', $step );
		$stepTitle = trim( $result[0]->nodeValue );

		$result = $this->xpath->query( './div', $step );
		$n = 0;
		$outstr = '';
		foreach ( $result as $elem ) {
			if ( $n > 0 ) {
				$outstr .= $this->parseElement( $elem );
			}
			$n++;
		}
		return '{"title":"' . $stepTitle . '","content":[' . rtrim( $outstr, ',' ) . ']},';
	}
	 ...
	 *
	 * NOTE: lesson= {"lesson":"Session #", "steps":[step, step, ...]}
	 *
	 * @since 0.1.0
	 *
	 * @param string $lesson_str HTML code for a lesson passed as a string.
	 * @return array decoding of JSON representation of the lesson.
	 */
	public function parse( $lesson_str ) {
		$doc = new DOMDocument();  // UTF-8 is default encoding
		$doc->loadHTML( $lesson_str );
		$this->xpath = new DOMXPath( $doc );
		//$context= $doc->getElementById('main');
		$context = $doc->documentElement;
		$result = $this->xpath->query( "//div[1]", $context );
		$lessonNum = trim( $result[0]->nodeValue );
		$steps = $context->getElementsByTagName( 'section' );

		$stepstr = '';
		foreach ( $steps as $step ) {
			$stepstr .= $this->parseStep( $step );
		}

		$outstr = '{"lesson":"' . $lessonNum . '","steps":[' . rtrim( $stepstr, ',' ) . ']}';

		return json_decode( $outstr ); // answer needs to be array (not JSON string)
	}
}
